...
interface NavBarLeftBtnProps {
  btnConfig: ButtonConfig;
}

const NavBarLeft: React.FC = () => {
  const projectsBtn: ButtonConfig = {
    btnText: 'Project',
    hasDropDown: true,
    link: '',
    dropDownMenu: [
...
const NavBarLeftBtn: React.FC<NavBarLeftBtnProps> = (props) => {
  const [clicked, setClicked] = useState(false)
  const transitionDuration = 200
  const btnRef = useRef<HTMLButtonElement>(null)
  const numberOfOptions = props.btnConfig.dropDownMenu.length
  const dropDownNodeRefs = useRef(props.btnConfig.dropDownMenu.map(() => createRef<HTMLDivElement>()));
  ...
  useEffect(() => {
    function handleClickOutside(event: MouseEvent) {
      if (clicked && btnRef.current && !btnRef.current.contains(event.target as Node)) {
        setClicked(false)
      }
    }
          ...
          >
            ...
            {props.btnConfig.dropDownMenu.map((dropDownOption, index) => (
              <CSSTransition
                key={index}
                in={clicked}
                nodeRef={dropDownNodeRefs.current[index]}
                timeout={{
                ...
                unmountOnExit
              >
                <div ref={dropDownNodeRefs.current[index]}>
                  {
...

                  {
                    dropDownOption.reactPage ?
                      <li className='DropDownOption'>
                        <Link to={dropDownOption.link} className='DropDownContent'>
                          {dropDownOption.optionText}
                        </Link>
                      </li>
                      :
